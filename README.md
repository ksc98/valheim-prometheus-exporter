...
Needs [BepInExPack_Valheim](https://thunderstore.io/c/valheim/p/denikson/BepInExPack_Valheim/) on the server.

1. Download `ValheimPrometheusExporter-<version>.zip` from the [latest release](https://github.com/ksc98/valheim-prometheus-exporter/releases/latest).
2. Unzip it and copy `ValheimPrometheusExporter.dll` into the server's `BepInEx/plugins/`.
...
[Collect]
IntervalSeconds = 5   # how often samples are updated
```
